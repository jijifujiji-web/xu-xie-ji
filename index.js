...
function extractAiResponseText(data) {
  const direct = (
    ...
    data?.choices?.[0]?.text ||
    data?.candidates?.[0]?.content?.parts?.map(p => p.text || p.content || "").join("") ||
    data?.candidates?.[0]?.content?.parts?.[0]?.text ||
    data?.candidates?.[0]?.text ||
    (Array.isArray(data?.content) ? data.content.map(x => typeof x === "string" ? x : (x?.text || x?.content || "")).join("") : data?.content) ||
  ...
  if (Array.isArray(direct)) return direct.map(x => typeof x === "string" ? x : (x?.text || x?.content || "")).join("");

  // 某些中转/Gemini兼容层会把正文塞到非标准字段里。这里做一次浅递归捞文本，避免误判“空回”。
  const seen = new Set();
  const chunks = [];
  const walk = (node, depth = 0) => {
    ...
    if (typeof node === "string") {
      const t = node.trim();
      if ((t.startsWith("{") && t.includes("characters")) || t.includes('"plot"') || t.includes('"world"')) chunks.push(t);
      return;
    }
    if (typeof node !== "object") return;
    seen.add(node);
    for (const [k, v] of Object.entries(node)) {
      if (["content", "text", "output", "response", "reasoning", "reasoning_content"].includes(k)) walk(v, depth + 1);
    }
    if (!chunks.length) {
...
const STRICT_WORLDBOOK_SYSTEM_PROMPT = `你是小说世界书分析器。你的唯一任务：读取小说文本，输出一个能被 JSON.parse 直接解析的 JSON 对象。

【绝对禁止】
- 禁止 Markdown、代码块、解释、标题、注释、道歉、自检报告、思考过程。
- 禁止在 JSON 外输出任何字符。
- 禁止尾随逗号、单引号、中文逗号充当结构分隔符、undefined、NaN、Infinity。
- 禁止输出半截 JSON。宁可少写条目，也必须完整闭合。
...
4. 字段之间必须使用英文逗号。
5. 数组元素之间必须使用英文逗号。
6. 字符串里不要出现真实换行；需要分隔时用中文分号。
7. 不确定的信息写 "文本未明确"。
8. 为避免截断，每类最多 5 条；如果内容很多，优先保留最新变化和影响后续续写的信息。
9. 输出前必须在内部执行 JSON.parse 级别自检，但自检内容绝不能输出。

【内部自检清单，禁止输出】
- characters / plot / world 是否存在且为数组。
- 每个对象是否完整闭合。
- 每个字段后面是否正确加英文逗号，最后一个字段无尾随逗号。
- 每个数组对象之间是否有英文逗号。
- 所有字符串是否闭合。
- 如果 JSON 过长，删除次要条目，确保最终 JSON 完整。`;
...
3. relationships 必须是字符串，不要写成对象，避免嵌套括号导致 JSON 损坏。
4. tags 必须是字符串。
5. 如果人物状态发生重大变化，例如背叛、死亡、失忆、修为突破、关系破裂、阵营变化，必须写入 status、relationships、experience、ability。
6. 如果新信息推翻旧设定，以最新文本为准，并在 content 或 status 中写明“最新变化”。
7. 输出语言保持中文。
8. 输出前在内部检查一遍：能不能直接 JSON.parse；如果不能，先修好再输出。不要把检查过程写出来。
...
async function analyzeWorldBookFromText(text) {
  ...
  const settings = extension_settings[extensionName] || {};
  const systemPrompt = settings.worldBookAnalysisSystemPrompt || STRICT_WORLDBOOK_SYSTEM_PROMPT;
  ...
  try {
    ...
    if (!isWorldBookEmpty(salvaged)) return salvaged;

    console.warn("[续写鸡] 世界书分析单次调用无有效结构，保存诊断条目并继续后续章节", {
      resultPreview: String(resultText || "").slice(0, 1500)
    });
    return createFallbackWorldBookFromRaw(resultText || "【世界书分析空回】", sourceText);
  } catch (err) {
