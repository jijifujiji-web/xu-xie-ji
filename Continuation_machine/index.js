...
function buildOriginalTextFor正文库() {
...
function buildGenerateConfig() {
  ...
  let fullStylePrompt = `文风参考【${styleName}】`;
  if (styleDesc) {
    fullStylePrompt += `，风格特点：${styleDesc}`;
  }

  if (!BUILT_IN_STYLES.includes(styleName)) {
    const customStyle = customStylesList.find(item => item.name === styleName);
    if (customStyle?.tags) {
      fullStylePrompt += `，写作倾向与禁忌：${customStyle.tags}`;
    }
    if (customStyle?.sample) {
      fullStylePrompt += `，参考片段语言质感：${customStyle.sample}`;
    }
  }
      ...
      <div class="xuxieji-modal-content style-control-modal-content">
        ...
        <div class="xuxieji-modal-body">
          <div class="style-control-tip">
            当前文风会直接影响续写 Prompt。你可以新增文风、保存修改、切换当前文风；内置文风可作为模板另存。
          </div>
    ...
    <div class="xuxieji-editor-container">
      <header class="xuxieji-header">
          ...
          <div class="header-right">
              ...
              <button class="header-icon-btn" title="正文库" id="export_content_btn">正文库</button>
          </div>
      </header>
      <!-- 设置弹窗 -->
...
function openXiaomengEditor() {
  ...
  editorDom.closest(".xuxieji-mask").addClass("show");
  restoreCursorToEnd(editorDom.find("#xuxieji_editor_textarea")[0]);
  console.log("[续写鸡] 编辑器已打开，版本v1.4.8 分段修复版");
}
function exportContentToFile(format = "txt", forceOriginal正文库 = false) {
  if (!editorDom || isEditorDestroyed) return;
  const content = forceOriginal正文库 ? buildOriginalTextFor正文库() : buildOriginalTextFor正文库();
...
if ($("#preset_source").length === 0) {
  ...
  console.log("[续写鸡] 扩展初始化完成，版本v1.4.8 分段修复版");
});
